...
public class StringUtils {
    ...
    private static final String XUEXIN_IMG_PARTURL = ".imxuexin.cn/";// 图片替换路径
        ...
        synchronized (algorithm.intern()) {
            ...
            if (digest == null) {
                try {
                  ...
                  /*  Log.error("Failed to load the " + algorithm
                            + " MessageDigest. "
...
//    }
    ...
    * @author:shanjianlong
    * @date: 2019/2/12 11:08
    */
    public static String randomNumberString(int length){
        if (length < 1) {
            return null;
        }
        // Create a char buffer to put random numbers in.
        char[] randBuffer = new char[length];
        for (int i = 0; i < randBuffer.length; i++) {
...
//            Log.warn("Unable to decode byte array using UTF-8: " + result);
     ...
     */
    public static boolean versionCompare(String userVersion,
        ...
        try {
            if (userVersion != null) {
                ...
                if (userVerArr.length > 2) {
                    userVerNum = Integer.valueOf(userVerArr[0]) * 100000
                            + Integer.valueOf(userVerArr[1]) * 100
                            + Integer.valueOf(userVerArr[2]);
                    supportVerNum = Integer.valueOf(supportVerArr[0]) * 100000
                            + Integer.valueOf(supportVerArr[1]) * 100
                            + Integer.valueOf(supportVerArr[2]);

                } else if (userVerArr.length > 1) {
                    userVerNum = Integer.valueOf(userVerArr[0]) * 100000
                            + Integer.valueOf(userVerArr[1]) * 100;
                    supportVerNum = Integer.valueOf(supportVerArr[0]) * 100000
                            + Integer.valueOf(supportVerArr[1]) * 100;
    ...
    /**
     * 是否全对（选择题）
     * @param candidateAnswer 考生答案可能多选，大小写均支持，eg:ABC或者A,B,c或者乱序c,DB
     * @param questionAnswer  答案也可能多选，大小写均支持，eg:ABC或者A,B,c
     * @return
     */
    public static boolean isRight(String candidateAnswer,String questionAnswer){
        if(candidateAnswer==null||"".equals(candidateAnswer.trim())||questionAnswer==null ||"".equals(questionAnswer.trim())){
            return false;
        }else{
            String[] candidateAnswers = null;   //作答记录
            String[] questionAnswers = null;    //试题答案
            //去除逗号和空格
            candidateAnswer = candidateAnswer.replaceAll(",","").replaceAll("，","").replaceAll(" ","");
            questionAnswer = questionAnswer.replaceAll(",","").replaceAll("，","").replaceAll(" ","");
            candidateAnswers = candidateAnswer.split("");
            questionAnswers = questionAnswer.split("");
            if(candidateAnswers!=null&&questionAnswers!=null){
                //利用Set的排重特性
                Set<String> questionSet = new HashSet<String>();
                for(String qAnswer:questionAnswers){
                    if(qAnswer!=null&&!"".equals(qAnswer.trim())) {
                        questionSet.add(qAnswer.toUpperCase());
                    }
                }
                for(String cAnswer:candidateAnswers) {
                    if(cAnswer!=null&&!"".equals(cAnswer.trim())&&questionSet.add(cAnswer.toUpperCase())==true){
                        //成功添加进答案集合说明有不在答案中的作答选项
                        return false;
                    }
                }
                //1，程序走到这里说明学生的选项都在正确答案中
                //利用Set的排重特性
                Set<String> candiDateSet = new HashSet<String>();
                for(String cAnswer:candidateAnswers){
                    if(cAnswer!=null&&!"".equals(cAnswer.trim())) {
                        candiDateSet.add(cAnswer.toUpperCase());
                    }
                }
                for(String qAnswer:questionAnswers) {
                    if(qAnswer!=null && !"".equals(qAnswer.trim()) && candiDateSet.add(qAnswer.toUpperCase())==true){
                        //成功添加进学生作答集合说明有不在答案中的作答选项
                        return false;
                    }
                }
                //2，程序走到这里说明正确答案都包含在了学生作答记录里，
                // 综合1,2上面逻辑，学生答题记录等于答案
                return true;
            }
        }
        return false;
     ...
     * @return
     */
    public static boolean isHalfRight(String candidateAnswer,String questionAnswer){
        if(candidateAnswer==null||"".equals(candidateAnswer.trim())||questionAnswer==null ||"".equals(questionAnswer.trim())){
            return false;
        }else{
            String[] candidateAnswers = null;   //作答记录
            ...
            if(candidateAnswers!=null&&questionAnswers!=null){
                ...
                if(questionSet.size()!=candiDateSet.size()) {
                    return true;
                }
            }
        }
    ...
    /**
     * 根据答案返回数组 eg:"ABC"返回{"A","B","C","AB","BC","CA","ABC"}
     * @param answer
     * @return
     */
    public static String[] getAnswerArray(String answer){
    ...
    /**
     * 输出答案的所有组合 eg:ABC则输出{A,B,C,AB,AC,BC,ABC}
     * @param answer
     */
    public static Object[] getAllCombo(String answer){
        String[] answers = null;
        if(answer!=null&&!"".equals(answer.trim())){
            answers = answer.split("");
        }
        List<String> resultList = new ArrayList<String>();
        combiantion(answers, resultList);
...
//        }
    ...
    public static void combiantion(String chs[], List<String> resultList) {
        if (chs.length == 0)
            return;

        Stack<String> stack = new Stack<String>();
        for (int i = 1; i <= chs.length; i++) {
            combine(chs, 0, i, stack, resultList);
        }
    }

    //从字符数组中第begin个字符开始挑选number个字符加入list中
    public static void combine(String[] chs, int begin, int number,
            Stack<String> stack, List<String> resultList) {
        if (number == 0) {
            String str = new String();
            for (String chara : stack) {
                str = str + chara;
            }
            resultList.add(str);
            return;
        ...
        combine(chs, begin + 1, number - 1, stack, resultList);
        stack.pop();
        combine(chs, begin + 1, number, stack, resultList);
    }


    public static void main(String[] args) {
        System.out.println(isMobile("16612345678"));
        System.out.println(isMobile("18912345678"));
        System.out.println(isMobile("15212345678"));
        System.out.println(isMobile("19812345678"));

        System.out.println("**************");
        for(int i = 0 ; i < 50 ; i++) {
            System.out.println(i+":"+randomNumberString(6));
        }
    }
}